...
# Lambda package: db_models.py sits alongside this file (Postgres edition)
from db_models import UserRecord as DBUser, PipelineRun as DBRun, get_db, init_db
...
# ── LAMBDA ADAPTER ────────────────────────────────────────────────────────────
# Mangum translates API Gateway events <-> ASGI so the SAME FastAPI `app` runs
# unchanged on Lambda. The Lambda handler string is "main.handler".
